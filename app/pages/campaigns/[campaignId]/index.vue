...
<template>
  <div class="space-y-8">
    <div v-if="pending" class="space-y-4">
      <UCard class="h-28 animate-pulse bg-slate-900/40" />
      <UCard class="h-40 animate-pulse bg-slate-900/40" />
    </div>
    ...
    <div v-else-if="campaign" class="space-y-6">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p class="text-xs uppercase tracking-[0.3em] text-slate-500">{{ campaign.system }}</p>
      ...
      <UCard class="border border-slate-800 bg-slate-900/40">
        <template #header>
          <div>
            ...
            <p class="text-sm text-slate-400">Update where the story left off.</p>
          </div>
        </template>
        <div class="space-y-4">
