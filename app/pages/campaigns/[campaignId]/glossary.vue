...
<template>
  <div class="space-y-8">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <p class="text-xs uppercase tracking-[0.3em] text-slate-500">Glossary</p>
    ...
    <div v-else-if="!entries?.length" class="rounded-xl border border-dashed border-slate-800 p-10 text-center">
      ...
      >
        <template #header>
          <div class="flex items-start justify-between gap-3">
            <div>
              <p class="text-xs uppercase tracking-[0.2em] text-slate-500">{{ entry.type }}</p>
              <h3 class="text-lg font-semibold">{{ entry.name }}</h3>
              <p v-if="entry.aliases" class="text-xs text-slate-400">Aliases: {{ entry.aliases }}</p>
            </div>
            <div class="flex gap-2">
        ...
        <div class="mt-4 space-y-2">
          <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Linked sessions</p>
          ...
          <div v-else class="text-xs text-slate-400">No sessions linked yet.</div>
          <div class="flex gap-2">
            <select
    ...
    <UModal v-model:open="isEditOpen">
      <template #content>
        <UCard class="border border-slate-800 bg-slate-950">
          ...
          <div class="space-y-4">
            <div>
              <label class="mb-2 block text-sm text-slate-300">Type</label>
              <select
            ...
            <div>
              <label class="mb-2 block text-sm text-slate-300">Aliases</label>
