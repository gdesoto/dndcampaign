...
Controls use Nuxt UI's native size variants, with `md` defaults in `app/app.config.ts`. Change a component's size scale or default there so labels, icons, padding, and embedded controls stay coordinated across the app. Use the `size` prop for an intentional instance size. Mobile controls no longer use forced minimum dimensions, extra number-input padding, or expanded checkbox pseudo-elements; review target spacing in the rendered layout.
