...
export const identities: Identity[] = [
  ...
  },
  {
    ...
    description:
      "A holiday-let console: teal on zinc, Fraunces titles over Nunito, generous radii and comfortable rows, light off the water on the page ground, and cards that lift into it under the cursor. Density gives way to warmth without giving up a single scanning rule.",
    icon: "i-lucide-palmtree",
    className: "identity-coastal",
    colors: { primary: "teal", neutral: "zinc" },
    ui: {
      card: { root: "ring-0 shadow-sm bg-elevated identity-lift" },
      table: { td: "h-11", th: "bg-elevated/60" },
      button: { base: "font-semibold" },
    },
    props: { badge: { variant: "soft" } },
    css: `/* app/assets/css/main.css */\n.identity-coastal {\n  --font-sans: var(--font-nunito);\n  --font-display: var(--font-fraunces);\n  --text-page-title: 1.375rem;\n  --spacing-page: 1.5rem;\n  --ui-radius: 0.875rem;\n}\n\n/* Character, at rest: light off the water, low and wide. */\n.identity-coastal .app-ground {\n  background-image:\n    radial-gradient(100% 55% at 8% 0%,\n      color-mix(in oklab, var(--ui-primary) 9%, transparent), transparent 55%),\n    linear-gradient(to bottom, transparent 45%,\n      color-mix(in oklab, var(--ui-primary) 7%, transparent) 100%);\n}\n\n/* Character, under the cursor: the card lifts into it. transform\n   composites, so nothing reflows; reduced motion drops the movement. */\n.identity-lift { transition: transform 220ms ease, box-shadow 220ms ease; }\n.identity-lift:hover {\n  transform: translateY(-2px);\n  box-shadow: 0 10px 22px -14px\n    color-mix(in oklab, var(--ui-primary) 55%, transparent);\n}`,
    config: `ui: {\n  colors: { primary: 'teal', neutral: 'zinc' },\n}`,
    theme: `<UTheme\n  :ui="{\n    card: { root: 'ring-0 shadow-sm bg-elevated identity-lift' },\n    table: { td: 'h-11', th: 'bg-elevated/60' },\n    button: { base: 'font-semibold' },\n  }"\n  :props="{ badge: { variant: 'soft' } }"\n/>`,
  },
];
