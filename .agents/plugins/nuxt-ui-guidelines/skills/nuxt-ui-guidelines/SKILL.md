...
- One clear page heading and at most one solid-primary action in the active context. Align comparison values and emphasize identity before metadata. Constant identity and varying semantic signals serve different purposes.
...
This folder is self-contained. Its guide and source snapshot come from `nuxt/ui-guidelines` on 2026-09-09. The full source app, gallery, fictional data, theme, tests, manifest, and lockfile are included under `assets/example-app/`; dependencies and generated artifacts are excluded. The single bundled specification is `references/guidelines.md`; the example app imports it directly. The external HTML remains provenance only. Keep [assets/example-app/LICENSE](assets/example-app/LICENSE) with copied source. Applying this skill does not require global installation, the external HTML reference, or changes to unrelated projects.
