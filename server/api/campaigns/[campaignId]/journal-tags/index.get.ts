...
export default defineEventHandler(async (event) => {
  ...
  const parsedQuery = campaignJournalTagListQuerySchema.safeParse(getQuery(event))
  if (!parsedQuery.success) {
    return fail(event, 400, 'VALIDATION_ERROR', 'Invalid journal tag query parameters')
  }
