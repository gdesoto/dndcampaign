...
export default defineEventHandler(async (event) => {
  ...
  const parsed = await readValidatedBodySafe(event, calendarTemplateApplySchema)
  if (!parsed.success) {
    return fail(event, 400, 'VALIDATION_ERROR', 'Invalid calendar template payload', parsed.fieldErrors)
  }
