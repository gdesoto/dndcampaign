...
import { campaignJournalArchiveInputSchema } from '#shared/schemas/campaign-journal'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = await readValidatedBodySafe(event, campaignJournalArchiveInputSchema)
  if (!parsed.success) {
    return fail(event, 400, 'VALIDATION_ERROR', 'Invalid archive payload', parsed.fieldErrors)
  }
