...
import { campaignJournalTransferInputSchema } from '#shared/schemas/campaign-journal'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = await readValidatedBodySafe(event, campaignJournalTransferInputSchema)
  if (!parsed.success) {
    return fail(event, 400, 'VALIDATION_ERROR', 'Invalid transfer payload', parsed.fieldErrors)
  }
