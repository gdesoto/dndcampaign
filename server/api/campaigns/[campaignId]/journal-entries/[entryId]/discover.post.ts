...
import { campaignJournalDiscoverInputSchema } from '#shared/schemas/campaign-journal'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = await readValidatedBodySafe(event, campaignJournalDiscoverInputSchema)
  if (!parsed.success) {
    return fail(event, 400, 'VALIDATION_ERROR', 'Invalid discover payload', parsed.fieldErrors)
  }
