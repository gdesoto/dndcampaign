...
import { campaignJournalUpdateSchema } from '#shared/schemas/campaign-journal'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = await readValidatedBodySafe(event, campaignJournalUpdateSchema)
  if (!parsed.success) {
    return fail(event, 400, 'VALIDATION_ERROR', 'Invalid journal update payload', parsed.fieldErrors)
  }
