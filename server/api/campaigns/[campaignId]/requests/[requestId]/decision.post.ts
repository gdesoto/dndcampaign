...
import { campaignRequestDecisionInputSchema } from '#shared/schemas/campaign-requests'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = await readValidatedBodySafe(event, campaignRequestDecisionInputSchema)
  if (!parsed.success) {
    return fail(event, 400, 'VALIDATION_ERROR', 'Invalid request decision payload', parsed.fieldErrors)
  }
