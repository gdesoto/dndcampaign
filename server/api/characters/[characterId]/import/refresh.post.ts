...
export default defineEventHandler(async (event) => {
  ...
  if (!access.canEdit) {
    return fail(event, 403, 'FORBIDDEN', 'You do not have permission to refresh imports for this character')
  }

  const parsed = await readValidatedBodySafe(event, characterImportRefreshSchema)
  if (!parsed.success) {
    return fail(event, 400, 'VALIDATION_ERROR', 'Invalid refresh payload', parsed.fieldErrors)
  }
